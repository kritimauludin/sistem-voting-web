...
@section('title', '[RB Vote - Kandidat]')
    ...
    <div class="col-lg-8">
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-dark d-inline">{{$polling[0]->title_polling}} - Calon {{$kandidat}}</h6>
            </div>
            <div class="card-body">
                <form action="/mypolling/kandidat" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" id="kandidat" name="kandidat" value="{{$kandidat}}">
                    <input type="hidden" id="polling_id" name="polling_id" value="{{$polling[0]->id}}">
                    <div class="mb-3">
                        <label for="title_kandidat" class="form-label">Nama Calon</label>
                        <input type="text" class="form-control @error('title_kandidat') is-invalid @enderror" id="title_kandidat" name="title_kandidat" value="{{old('title_kandidat')}}" required autofocus>
